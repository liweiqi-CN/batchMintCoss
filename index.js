...
async function performTransaction(walletInfo, numberOfTimes) {
    ...
    for (let i = 0; i < numberOfTimes; i++) {
        try {
            ...
            console.log(`${account.address}, 第 ${i + 1} 次操作成功: ${'https://celestia.explorers.guru/transaction/' + result.transactionHash}`);
        } catch (error) {
...
async function main() {
    ...
    // });
    Promise.all(walletData.map(wallet => performTransaction(wallet, 1)))
        .then(() => {
            console.log("所有操作完成");
        })
        .catch(error => {
